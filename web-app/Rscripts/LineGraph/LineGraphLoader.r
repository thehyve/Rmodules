...
HDD.data.type
    ...
    if (!plot.individuals) {
        p <- p + geom_errorbar(limits,
                width = timeDiff * errorBarScale - (errorBarScale / timeDiff) * 2)
    }

    #Defines a continuous x-axis with proper break-locations, labels, and axis-name
    p <- p + scale_x_continuous(name = "TIMEPOINT", breaks = dataOutput$TIME_VALUE,
            labels = dataOutput$TIMEPOINT, expand = c(0, timeDiff * errorBarScale))
