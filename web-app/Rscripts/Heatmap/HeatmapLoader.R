...
plotHeatmap <- function(data, colcolors, color.range.clamps, output.file = "Heatmap", extension = "png") {
    ...
    hmCanvasDiv <- list(xLeft = leftMarginSize / imageWidth, xMain = mainWidth / imageWidth,
            xRight = rightMarginSize / imageWidth, yTopLarge = topSpectrumHeight / imageHeight,
            yTopSmall = topMarginSize / imageHeight, yMain = mainHeight / imageHeight,
            yBottom = bottomMarginSize / imageHeight)

    if (extension == "svg") {
        CairoSVG(file = paste(output.file, ".svg", sep = ""), width = imageWidth / 200, height = imageHeight / 200,
                pointsize = hmPars$pointSize * 0.35)
    } else {
        CairoPNG(file = paste(output.file, ".png", sep = ""), width = imageWidth, height = imageHeight,
                pointsize = hmPars$pointSize)
    }
    par(mar = c(0, 0, 0, 0))
    heatmap.2(data,
            Rowv = NA,
            ...
            key = TRUE,
            keysize = 0.001,
            density.info = "histogram",  # options are density.info = c("histogram", "density", "none")
            trace = "none",
