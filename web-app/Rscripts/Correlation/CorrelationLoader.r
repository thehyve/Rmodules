...
)
{
    ...
    if (correlation.by == "subject") {
        write.table(format(cor(line.data, method = correlation.method), digits = 3),
                correlationResultsFile, quote = F, sep = "\t", row.names = F, col.names = T)
    }

    if (correlation.by == "variable") {
        write.table(format(cor.pvalue(line.data, method = correlation.method), digits = 5),
                correlationResultsFile, quote = F, sep = "\t", row.names = F, col.names = T)
